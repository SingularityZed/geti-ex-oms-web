...
const NumberInfo: React.FC<NumberInfoProps> = ({
  ...
  >
    ...
    <div className={styles.numberInfoValue} style={gap ? { marginTop: gap } : {}}>
      <span>
        {total}
        {suffix && <em className={styles.suffix} style={{marginLeft:'14%',fontSize:'12px'}}>{suffix}</em>}
      </span>
      {(status || subTotal) && (
        <span className={styles.subTotal} style={{marginTop:'5px',marginLeft:'-8%'}}>
          {status && status === 'up' ? <CaretUpOutlined /> : <CaretDownOutlined />}
          <em  style={{fontSize:'12px'}}>{subTotal}</em>
        </span>
      )}
    </div>
  </div>
